...
To avoid a longer training time~\cite{schirrmeister-deep-2017} also used dilated convolution.
...
\end{subfigure}
\caption[Receptive field comparison]{Both graphs \textbf{(a)} and \textbf{(b)} show the receptive fields (delimited by red lines) \todo{nn names} used for making one prediction (yellow dot on the x axis) with respect to one complete 1200 ms long input window which is given to the networks.
