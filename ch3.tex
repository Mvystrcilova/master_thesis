...
By receptive field we mean the part of the input of the network which affects one specific output (in our case it corresponds to one crop from which one prediction is made).
...
Since we are investigating the frequency bands and their effective utilization, we inspect this difference as a possible reason for why the network, when employed to predict velocity and absolute velocity of hand movement, does presumably not use high-gamma.
