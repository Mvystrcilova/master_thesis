...
\caption[Deep4Net architecture]{The architecture of the Deep4Net modified to be suitable for regression as used in \cite{Hammer-2021}}
...
In~\cite{Hammer-2021} and \cite{schirrmeister-deep-2017} this parameter was set to \texttt{False (sbp0)}. The only analysis where the \texttt{stride\_before\_pool} was set to \texttt{True (sbp1)} was when analyzing the gradient peak and also to study the dependence of the size of the receptive field on the performance of the network. See Figure \ref{fig:figure-distance} . \\
