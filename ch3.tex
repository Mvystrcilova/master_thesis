...
The original Deep4Net corresponds to the {variable}_k3_d3_sbp0 in Table \ref{Table}.
The suffix \textit{sbp} refers to a parameter of the Deep4Net namely the \textit{stride\_before\_pool} parameter.
This parameter influences the dilations in the max-pool layers of the network. In Hammer et al \ref{cite} this parameter was set to False (sbp0), nevertheless, in the gradient peak analysis performed by their group which we are referring to in Section \{experiments_gradient_peak}, the studied network had ‘stride_before_pool’ set to True (sbp1).
