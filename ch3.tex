...
The participants performed a motor task consisting of driving a car in a computer simulation.
...
We performed some modifications to see how the network's performance/gradients change.
...
A high-pass filtered dataset using a 15th order Butterworth filter, cut-off frequency 60Hz, non-zero phase shift and a low-pass dataset using Butterworth filter order 15, cut-off frequency 40Hz, non-zero phase shift.
\\

\item \textbf{Shifting} Originally in Hammer et al.~\cite{Hammer-2021} the dataset was constructed so that the predictions were made from signals recorded prior to their execution.
In addition to this, we also created datasets where the labels (i.e. values of kinematic variables) were shifted so that predictions were made also from signals recorded after movement execution.
While a network trained in this manner is unsuitable for online BCI, it allows us to inspect how the frequency bands utilized by it change with shifting and gives insight into the networks behaviour.
...
The whitened signal was iqr-normalized.
...
Besides the describing the architecture as used in \cite{Hammer-2021} we focus on how cropped decoding is used \ref{subsec:cropped-decoding}, how classification and regression differ and what it means with respect to the architecture, its inputs and outputs \ref{subsec:difference-between-deep4net-for-classification-and-regression}, what the receptive field of the network looks like and how it affects the predictions \ref{subsec:receptive-field}, we describe the training procedure \ref{subsec:training}, the visualization of the networks gradients \ref{subsec:gradinet-visualization}, and architectural modifications we introduce in this thesis \ref{subsec:architectural-modifications}. Lastly we also state how performance is analyzed \ref{subsec:performance-analysis}.
...
To use this network for regression which is what we are doing in this thesis, the only modification that needs to be done is removing the last softmax layer and replacing it with a convolution (conv\_classifier).
...
To avoid a longer training time, Schirrmeister et al.\cite{schirrmeister-deep-2017} introduce trick with dilated convolution can be implemented.
...
In the case of regression we can also create an input window of a certain length and the label it with the corresponding value of the kinematic variable.
...
\caption[Receptive field]{The receptive field of the original Deep4Net, in this thesis denoted as {variable}\_k3\_d3\_sbp0 in the context of one whole input window. The vertical red lines delimit the first receptive field (crop) of the CNN which is used for calculating the first prediction. The green line represents the centre of the receptive field and the yellow dot on the x axis the first time-point that is being predicted. The gradient bar on the right shows the number of times each input sample was used in the calculations of the output.}
...
But the CNN mostly focuses on signals around the centre of the receptive field and therefore, network pays little attention to the data that directly precede the movement. \\
...
The changed role of the receptive field between classification and regression might be a reason the network when employed to do regression does not use high-gamma.
...
Except for the learning rate, we left all hyper-parameters equal to those in Hammer et al. \cite{Hammer-2021}, because it was our goal to reproduce their results and work from there.
...
In Schirrmeister et al. \cite{schirrmeister-deep-2017} and Hammer et\ al. \cite{Hammer-2021} the main visualization method was input-perturbation output correlation.
