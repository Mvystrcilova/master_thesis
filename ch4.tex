...
To also account for the increase of the 83.33Hz frequency in the output, we studied how the signal is affected by max-pool layers only.
...
\begin{figure}[!hpbp]
\begin{subfigure}[a]{\textwidth}
   \includegraphics[width=0.8\linewidth]{img/ch4/vel-shifting-performance-comparison}
   \caption{}
\end{subfigure}\label{fig:vel-shifting-performance}

\begin{subfigure}[b]{\textwidth}
   \includegraphics[width=0.8\linewidth]{img/ch4/absVel-shifting-performance-comparison}
   \caption{}
...
\caption[Spectral whitening - gradients]{Gradients of networks trained to decode velocity \textbf{(a)} and absolute velocity \textbf{(b)} on spectraly-whitened datasets}
