...
The classifiers include Support vector machines (SVM) \cite{svm-alg}, Linear Discriminant analysis (LDA) \cite{lda-paper}, Quadratic discriminant analysis (QDA) \cite{qda-paper} and their adaptive versions which have gained popularity in the last ten years \cite{2\_eeg\_review} and probabilistic methods such as conditional random fields \cite{} or naive bayes classifiers. \\
...
Neural networks which we describe separately in Section \ref{sec:ddn-decoding} have also been proposed to solve movement related regression tasks.
...
\section{DNNs for (i)EEG decoding}\label{sec:dnn-decoding}
